...
{ricci} uses nonstandard evaluation to specify tensor indices (see [.()]). The main purpose is to safe a lot of quotation marks which otherwise would clutter the code (`i` instead of `"i"`). In addition this also allows to specify some operations in more direct way (e.g. substitution is specified as `subst(x, i -> f)` is short for substitute index `i` with `f`).

A key point about specifying the index labels is that we can make use of [Ricci calculus](https://en.wikipedia.org/wiki/Ricci_calculus) conventions and pack a bunch of different multiplications into one operator (`*`). There are no separate "inner product", "outer product", "dot product", "Kronecker product" or "element-wise product". All are represented by a single product `*`. The arrangement of the indices determines what product is carried out, as is in the spirit of [Ricci calculus](https://en.wikipedia.org/wiki/Ricci_calculus).
